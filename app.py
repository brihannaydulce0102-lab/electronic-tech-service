...
if not st.session_state.logged_in:
    ...
    if st.button("Entrar", type="primary"):
        user = usuarios[(usuarios["usuario"] == usuario) & (usuarios["contraseña"] == contraseña)]
        if not user.empty:
...
# ==================== MENÚ ====================
menu_opciones = ["🏠 Inicio", "📋 Nueva Reparación", "📋 Ver Órdenes", "🔍 Buscar", 
                 "📄 Cotizaciones", "🖨️ Imprimir Recibo"]
...
# ==================== VER ÓRDENES ====================
    ...
    if not df.empty:
        ...
        if st.session_state.rol == "admin":
            st.subheader("✏️ Editar o Eliminar (Solo Administrador)")
            id_accion = st.number_input("ID de la Orden", min_value=1, step=1)
            accion = st.radio("Acción", ["Eliminar Orden"])
            ...
            st.info("Solo el administrador puede editar o eliminar órdenes")
    else:
        ...
        if st.button("Crear Usuario"):
            if nuevo_usuario and nueva_contraseña:
                if nuevo_usuario in usuarios["usuario"].values:
                    ...
                    nuevo = pd.DataFrame([{"usuario": nuevo_usuario, "contraseña": nueva_contraseña, "rol": nuevo_rol}])
